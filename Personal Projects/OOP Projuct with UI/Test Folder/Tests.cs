        ...
        public void Sorter_SetSortingParameter_SortByGivenParameter(string sort)
        {
            ...
            switch (sort.ToLower())
            {
                ...
                case "price":
                    ...
                    break;
                case "restock cost":
                    tempProducts.OrderBy(item => item.RestockPrice).ThenBy(item => item.Id).ToList();
                    break;
                case "quantity":
                    tempProducts.OrderBy(item => item.Quantity).ThenBy(item => item.Id).ToList();
                    break;
                default:
                    break;
            }
